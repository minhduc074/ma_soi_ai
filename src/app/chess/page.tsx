...
import { runChessLoop } from '@/lib/chess/engine';
import { ChessPhase, ChessChatMessage, ChessApiLogEntry } from '@/lib/chess/types';
import ChessScene3D from '@/components/ChessScene3D';

/* ------------------------------------------------------------------ */
/*  Constants                                                          */
/* ------------------------------------------------------------------ */
const PHASE_LABELS: Record<ChessPhase, string> = {
  setup: '⚙️ Chuẩn bị',
  playing: '♟️ Đang chơi',
  check: '⚠️ Chiếu',
  checkmate: '♚ Chiếu hết',
...
/*  MessageBubble                                                      */
/* ------------------------------------------------------------------ */
function MessageBubble({ msg }: { msg: ChessChatMessage }) {
  if (msg.type === 'system') {
    return (
...
function ApiLogPanel() {
  const apiLogs = useChessStore((s) => s.apiLogs);
...
function ChessPageContent() {
  ...
  const logs = useChessStore((s) => s.logs);
  const isRunning = useChessStore((s) => s.isRunning);
  const setRunning = useChessStore((s) => s.setRunning);
  const speed = useChessStore((s) => s.speed);
  const setSpeed = useChessStore((s) => s.setSpeed);
  const thoughtProbability = useChessStore((s) => s.thoughtProbability);
  const setThoughtProbability = useChessStore((s) => s.setThoughtProbability);
  const ttsEnabled = useChessStore((s) => s.ttsEnabled);
  const setTtsEnabled = useChessStore((s) => s.setTtsEnabled);
  const apiLogs = useChessStore((s) => s.apiLogs);
    ...
    if (!players) {
      initGame(
        ...
        },
        {
          ...
          model: 'gemini/gemini-2.0-flash-exp',
          provider: 'gemini',
          personality: 'Tactical calculator',
        },
        'ai_vs_ai'
      );
    }
  ...
  const currentPlayerObj = players?.find(p => (p.color === 'white') === (currentTurn === 'w'));
      ...
      <header className="sticky top-0 z-50 backdrop-blur-md bg-black/40 border-b border-gray-700/50">
        <div className="max-w-[1400px] mx-auto px-3 md:px-4 py-2 flex items-center justify-between">
          <div className="flex items-center gap-2 md:gap-3">
            <h1 className="text-sm md:text-base font-bold">♟️ Cờ Vua AI</h1>
            <span className="text-xs px-2 py-0.5 rounded-full font-medium bg-slate-800 text-slate-200">
              {PHASE_LABELS[phase]}
            </span>
            <span className="text-xs text-gray-400">Nước {moveHistory.length}</span>
            {currentPlayerObj && !isGameOver && (
              <span className={`text-xs px-2 py-0.5 rounded ${currentTurn === 'w' ? 'bg-gray-300 text-gray-900' : 'bg-gray-700 text-gray-200'}`}>
                {currentPlayerObj.expression} {currentPlayerObj.name}
              </span>
            )}
            {chess?.isCheck() && !isGameOver && (
              <span className="text-xs px-2 py-0.5 rounded bg-red-800 text-red-200 animate-pulse">
                ⚠️ Chiếu!
              </span>
            )}
          </div>
      ...
      <div className="max-w-[1400px] mx-auto px-3 md:px-4 py-3 flex flex-col md:flex-row gap-3 md:gap-4 md:h-[calc(100vh-48px)]">
        ...
        <div className="md:flex-1 flex flex-col min-w-0 gap-2">
          ...
          <div className="flex gap-2 overflow-x-auto pb-1 flex-shrink-0">
            {players?.map((p) => {
              const isCurrentPlayer = (p.color === 'white') === (currentTurn === 'w');
              ...
              return (
                ...
                >
                  ...
                  <div className="flex flex-col min-w-[60px]">
                    ...
                    <span className={`text-[11px] font-mono ${p.color === 'white' ? 'text-gray-300' : 'text-gray-500'}`}>
                      {p.color === 'white' ? '⬜' : '⬛'} {p.color}
                    </span>
                    {p.isHuman && (
