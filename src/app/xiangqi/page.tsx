...
import { runXiangqiLoop } from '@/lib/xiangqi/engine';
import { XiangqiPhase, XiangqiChatMessage, XiangqiApiLogEntry } from '@/lib/xiangqi/types';
import XiangqiScene3D from '@/components/XiangqiScene3D';

/* ------------------------------------------------------------------ */
/*  Constants                                                          */
/* ------------------------------------------------------------------ */
const PHASE_LABELS: Record<XiangqiPhase, string> = {
  setup: '⚙️ Chuẩn bị',
  playing: '🐉 Đang chơi',
  check: '⚠️ Chiếu tướng',
  checkmate: '👑 Chiếu bí',
...
/*  MessageBubble                                                      */
/* ------------------------------------------------------------------ */
function MessageBubble({ msg }: { msg: XiangqiChatMessage }) {
  if (msg.type === 'system') {
    return (
...
function ApiLogPanel() {
  const apiLogs = useXiangqiStore((s) => s.apiLogs);
...
function XiangqiPageContent() {
  ...
  const moveHistory = useXiangqiStore((s) => s.moveHistory);
  const winner = useXiangqiStore((s) => s.winner);
  const logs = useXiangqiStore((s) => s.logs);
  const isRunning = useXiangqiStore((s) => s.isRunning);
  const setRunning = useXiangqiStore((s) => s.setRunning);
  const speed = useXiangqiStore((s) => s.speed);
  const setSpeed = useXiangqiStore((s) => s.setSpeed);
  const thoughtProbability = useXiangqiStore((s) => s.thoughtProbability);
  const setThoughtProbability = useXiangqiStore((s) => s.setThoughtProbability);
  const ttsEnabled = useXiangqiStore((s) => s.ttsEnabled);
  const setTtsEnabled = useXiangqiStore((s) => s.setTtsEnabled);
  const apiLogs = useXiangqiStore((s) => s.apiLogs);
  const gameMode = useXiangqiStore((s) => s.gameMode);
  const initGame = useXiangqiStore((s) => s.initGame);
  ...
  const isGameOver = ['checkmate', 'stalemate', 'draw'].includes(phase);
  const currentPlayerObj = players?.find(p => p.color === currentTurn);

  return (
    <div className="min-h-screen bg-gradient-to-br from-red-950 via-gray-950 to-red-950 text-white">
      {/* Header */}
      <header className="sticky top-0 z-50 backdrop-blur-md bg-black/40 border-b border-gray-700/50">
        <div className="max-w-[1400px] mx-auto px-3 md:px-4 py-2 flex items-center justify-between">
          <div className="flex items-center gap-2 md:gap-3">
            <h1 className="text-sm md:text-base font-bold">🐉 Cờ Tướng AI</h1>
            <span className="text-xs px-2 py-0.5 rounded-full font-medium bg-red-900 text-red-200">
              {PHASE_LABELS[phase]}
            </span>
            <span className="text-xs text-gray-400">Nước {moveHistory.length}</span>
            {currentPlayerObj && !isGameOver && (
              <span className={`text-xs px-2 py-0.5 rounded ${currentTurn === 'red' ? 'bg-red-800 text-red-200' : 'bg-gray-700 text-gray-200'}`}>
                {currentPlayerObj.expression} {currentPlayerObj.name}
              </span>
            )}
          </div>
          <div className="flex items-center gap-2 md:gap-3">
      ...
      <div className="max-w-[1400px] mx-auto px-3 md:px-4 py-3 flex flex-col md:flex-row gap-3 md:gap-4 md:h-[calc(100vh-48px)]">
        ...
        <div className="md:flex-1 flex flex-col min-w-0 gap-2">
          ...
          <div className="flex gap-2 overflow-x-auto pb-1 flex-shrink-0">
            {players?.map((p) => {
              const isCurrentPlayer = p.color === currentTurn;
              ...
              return (
                ...
                >
                  ...
                  <div className="flex flex-col min-w-[60px]">
                    ...
                    <span className={`text-[11px] font-mono ${p.color === 'red' ? 'text-red-400' : 'text-gray-400'}`}>
                      {p.color === 'red' ? '紅' : '黑'} {p.color}
                    </span>
                    {p.isHuman && (
